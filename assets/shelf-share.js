...
$('#shelf-share').onclick=async()=>{
 ...
 ctx.save();ctx.translate(0,125);ctx.scale(1280/960,252/(540*.35));if(!EventSystem.selected('shelf'))for(let n=0;n<10;n++)window.Customize?.shelf(ctx,164+n%5*139,n<5?58:133,106,Missions.data.selected.shelf);const shelf=EventSystem.selected('shelf');if(shelf){const im=EventSystem.asset(shelf.asset);await im.decode();const box=shelf.placement;for(let n=0;n<10;n++)ctx.drawImage(im,164+n%5*139+box[0],(n<5?58:133)+box[1],box[2],box[2]*im.naturalHeight/im.naturalWidth)}ctx.restore();
