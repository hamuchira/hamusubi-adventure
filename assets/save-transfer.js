...
function validate(raw){
 ...
 if(m.notifications!==undefined){if(!obj(m.notifications)||['customSeen','adventureSeen','played'].some(k=>!Array.isArray(m.notifications[k])||m.notifications[k].some(v=>typeof v!=='string')))throw Error('通知記録が正しくありません。')}else m.notifications={customSeen:[],adventureSeen:[],played:[]};if(raw.events!=null){const e=raw.events;if(e.version!==1||!obj(e.records)||!obj(e.owned)||!obj(e.selected)||!Array.isArray(e.seen))throw Error('イベント記録が正しくありません。');for(const r of Object.values(e.records))if(!obj(r)||!numbers(r.top)||!obj(r.items)||!['plays','clears','bestScore','bestRice'].every(k=>Number.isSafeInteger(r[k])))throw Error('イベントの進捗が正しくありません。');if(e.changes!==undefined&&(!obj(e.changes)||!['fur','furniture'].every(k=>Number.isSafeInteger(e.changes[k])&&e.changes[k]>=0)))throw Error('イベント記録が正しくありません。');if(e.titles!==undefined&&(!obj(e.titles)||Object.values(e.titles).some(t=>!obj(t)||!Number.isFinite(Date.parse(t.acquiredAt)))))throw Error('イベント記録が正しくありません。');if(e.campaignStats!==undefined&&(!obj(e.campaignStats)||Object.values(e.campaignStats).some(r=>!obj(r)||!Number.isSafeInteger(r.rainbow)||r.rainbow<0)))throw Error('イベント記録が正しくありません。');if(e.equippedTitle!=null&&(typeof e.equippedTitle!=='string'||!e.titles?.[e.equippedTitle]))throw Error('イベント記録が正しくありません。');}return clone(raw);
}
function apply(raw){
